...
# RNN cell [400, 400] <keras.layers.rnn.legacy_cells.MultiRNNCell object at 0x000002516F29A4A0>
# RNN cell [400, 400] <keras.layers.rnn.legacy_cells.MultiRNNCell object at 0x00000251051AB9D0>
# RNN cell [400, 400] <keras.layers.rnn.legacy_cells.MultiRNNCell object at 0x00000251051C0B80>    
...
def linear(inp, output_dim, scope=None, stddev=1.0, reuse_scope=False):
    ...
    """
    norm = tf.compat.v1.random_normal_initializer(stddev=stddev, dtype=tf.float32)
    const = tf.compat.v1.constant_initializer(0.0, dtype=tf.float32)
    with tf.compat.v1.variable_scope(scope or 'linear') as scope:
        scope.set_regularizer(tf.keras.regularizers.l2(l=0.5 * (REG_SCALE)))
        if reuse_scope:
            scope.reuse_variables()
        w = tf.compat.v1.get_variable('w', [inp.get_shape()[1], output_dim], initializer=norm, dtype=tf.float32)
        b = tf.compat.v1.get_variable('b', [output_dim], initializer=const, dtype=tf.float32)
...
class RNNGAN(object):
    ...
    def __init__(self, is_training, num_song_features=None, num_meta_features=None, conditioning='multi'):
        ...
        self._input_songdata = tf.compat.v1.placeholder(shape=[None, songlength, num_song_features], dtype=tf.float32,
                                              name="input_data")
        if conditioning is not 'none':
            self._input_metadata = tf.compat.v1.placeholder(shape=[None, songlength, num_meta_features], dtype=tf.float32,
                                                  name="input_metadata")
            self._input_metadata_wrong = tf.compat.v1.placeholder(shape=[None, songlength, num_meta_features], dtype=tf.float32,
        ...
        with tf.compat.v1.variable_scope('G') as scope:
            ...
            prev_target = tf.random.uniform(shape=[batch_size, num_song_features], minval=0.0, maxval=1.0,
                                            dtype=tf.float32)

            # REAL GENERATOR:
            if BIDIRECTIONAL_G:
                outputs, g_state_fw, g_state_bw = tf.compat.v1.nn.static_bidirectional_rnn(g_cell_fw, g_cell_bw,
                                                                                          random_rnninputs,
                                                                                          initial_state_fw=
                ...
                generated_point = tf.random.uniform(shape=[batch_size, num_song_features], minval=0.0, maxval=1.0,
                                                    dtype=tf.float32)
        ...
        reg_loss = tf.compat.v1.Print(reg_loss, reg_losses, 'reg_losses = ', summarize=20)
        # ---BEGIN, PRETRAINING. ---
        self.rnn_pretraining_loss = tf.reduce_mean(
            tf.math.squared_difference(x=tf.transpose(tf.stack(self._generated_features_pretraining), perm=[1, 0, 2]),
...
# <keras.layers.rnn.legacy_cells.MultiRNNCell object at 0x00000251051AB9D0> real
# shape, decisions: (None, 20, 1)
# <keras.layers.rnn.legacy_cells.MultiRNNCell object at 0x00000251051AB9D0> wrong
# shape, decisions: (None, 20, 1)
# <keras.layers.rnn.legacy_cells.MultiRNNCell object at 0x00000251051AB9D0> generated
# shape, decisions: (None, 20, 1)
        ...
        if conditioning == 'none' or LOSS_WRONG_D is False:
            ...
            self.d_loss = tf.reduce_mean(-2 * tf.math.log(tf.clip_by_value(self.real_d, 1e-1000000, 1.0))
                                         - tf.math.log(1 - tf.clip_by_value(self.generated_d, 0.0, 1.0 - 1e-1000000))
                                         - 2 * tf.math.log(1 - tf.clip_by_value(self.wrong_d, 0.0, 1.0 - 1e-1000000)))
        self.g_loss_feature_matching = tf.reduce_sum(
            tf.math.squared_difference(self.real_d_features, self.generated_d_features))
        self.g_loss = tf.reduce_mean(tf.math.log(tf.clip_by_value(1-self.generated_d, 1e-1000000, 1.0)))
        ...
        d_optimizer = tf.compat.v1.train.GradientDescentOptimizer(self._lr * D_LR_FACTOR)
        ...
        self._new_lr = tf.compat.v1.placeholder(shape=[], name="new_learning_rate", dtype=tf.float32)
    ...
    def discriminator(self, inputs, cell_fw, cell_bw, is_training, msg='', keep_prob=1.0):
        ...
        if not UNIDIRECTIONAL_D:
            ...
            outputs, state_fw, state_bw = tf.compat.v1.nn.static_bidirectional_rnn(cell_fw, cell_bw, inputs,
                                                                                  initial_state_fw=
        ...
        decision = tf.reduce_mean(decisions, axis=[1, 2])
        decision = tf.compat.v1.Print(decision, [decision], '{} decision = '.format(msg), first_n=20)
    ...
    with tf.Graph().as_default(), tf.compat.v1.Session(config=config) as session:
        ...
        for i in range(global_step, MAX_EPOCH):
            ...
            if i % 5 == 0:
                print("Saving model")
                try:
                    #E:\Codes\Deep Learning\20200919Updatedyylab1Release1\20200919Updatedyylab1Release1\0919
                    # os.path.join(baseDir, r"saved_gan_models\epoch_models\model_epoch")
